...
#pragma omp for
			for (int i = 0; i < colony_size; i++)
			{
				...
				CopyPopulation(new_sol, &pop[i + colony_size], num_cds, len_amino_seq);
...
#pragma omp single
			{
				SortbyRankCrowding(pop, 2 * colony_size, num_cds, len_amino_seq);
...
#pragma omp for		
			for (int i = 0; i < colony_size; i++)
			{
				...
				if (check)
					...
					sel_sol->counter++;
					CopyPopulation(sel_sol, &pop[i + colony_size], num_cds, len_amino_seq);
				}
				FreePopulation(new_sol, 1, num_cds);
			}
...
#pragma omp for		
			...
			{
				if (pop[i].counter > limit && !pop[i].flag)
				{
